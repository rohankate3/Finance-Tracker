...
function TransactionsTable({ transactions , addTransaction,fetchTransactions}) {
    ...
    // Filter the transactions based on search and typeFilter
    let transactionsFilter = transactions.filter((item) =>
        item.name.toLowerCase().includes(search.toLowerCase()) && item.type.includes(typeFilter)
    );

    // Sort the transactions based on sortKey
    const sortedTransactions = [...transactionsFilter].sort((a, b) => {
        if (sortKey === 'date') {
